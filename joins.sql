...
SELECT users.first_name, users.last_name, posts.id, posts.title, posts.url, posts.content, posts.created_at, posts.updated_at
FROM users
    INNER JOIN posts
...
SELECT users.username, posts.id, posts.title, posts.url, posts.content, posts.created_at, posts.updated_at
FROM users
    INNER JOIN posts
    ON users.id = posts.users_id
...
SELECT comments.id, comments.body, comments.created_at, comments.updated_at, posts.title AS "Post Title"
FROM comments
    LEFT JOIN posts
...
-- Additional 1. Count how many comments have been written on posts that have been created after July 14, 2015 ( should have one result, the value of the count should be 27)
SELECT COUNT(*)
FROM comments
    INNER JOIN posts
    ON comments.posts_id = posts.id
WHERE posts.created_at > '2015-07-14 00:00:00';

-- Additional 2. Find all users who comment about 'programming' ( should have 337 results)
SELECT *
FROM users
    RIGHT JOIN comments
